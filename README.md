...
- [Data](#data)
  ...
  - [power](#on-power-3)
- [Followup directions](#followup-directions)
...
# Followup directions
Here I'll describe some followup ideas.
The first followup, which we began work on, is the multi-scale spectrograms followup - representing each time-stamp with several spectrograms of different durations (0.5, 1 and 2s instead of just the 2s spectrograms used originally). The motivation for this is that perhaps spectrograms of different durations can detect patterns of different durations (specifically, we noticed that our search failed to detect many of the short injected signals, and a possible cause was too-low temporal resolution). In this followup, each of the three channels of the CNN corresponds to each of the scales. We didn't perform the entire process described above for this followup, instead, after training a CNN on a multi-scale version of the labeled data set, we moved to the evaluation phase, and examined where injected simulated signals are mapped to, and their detection rates using the thresholds chosen in the original pipeline. The resulting map space, and detection rates were very similar to the original pipeline so the conclusion is that this probably not the most promising direction to follow (technically, it is possible that when going through the entire pipeline the thresholds will change and the performance will be better, but it is unlikely to be significant).
An important product of this followup, however, is the slightly modified conditioning process. In the original project, the spectrograms were generated by plotting the Q-transform and saveing the plot as an RGB image (and when processing the spectrograms through the CNN, the channels corresponded to the colors), therefore the cutoffs are set in the conditioning process (by choosing the `vmin` and `vmax` parameters when plotting the spectrogram, set to be 0 and 25.5 respectively, in the `img_qtransform` function in `tools_gs_par.py`). In the multi-scale version, the spectrogram values are saved directly, and not saved as an RGB plot, and the they are not cutoff, so the cutoffs can be chosen during the training phase.
In future followups, this should be the preferred conditioning process, so they can be adapted from the conditioning code of the multi-scale followup, which can be found in [this repository](https://github.com/TomMarianer/multi-scale-gwml). The relevant functions are `load_condition_multi_scale` and `qsplit_multi_scale` found in [astrophys/tools/tools_gs_par.py](https://github.com/TomMarianer/multi-scale-gwml/blob/main/astrophys/tools/tools_gs_par.py).

Other than this, there are possible improvements to be made to different aspects of the pipeline - multi-detector incorporation, training, outlier detection and conditioning.
- Multi-detector incorporation:
  - Current state - coincident - search for outliers in each detector seperately, and then check if there are any outliers coincident in both detectors (which there are none in the data we processed).
  - Possible improvement - coherent search - convert the search to a coherent one instead of coincident. Attempt ot detect outliers while considering the spectrograms of multiple detectors. A natural way to adapt our search for this purpose is to assign each channel of the CNN to a different network. Currently, since there are only two detectors (for most of the available data, and in the near future there will be only two detectors with high sensitivity, since Virgo is much less sensitive than the two LIGO detectors), and the CNN has three channels, there is some freedom in what to insert into the third channel, perhaps set it to 0, or insert some average spectrogram. Training the CNN for a coherent search will require adapting the labeled set to a coherent one, where for each time-stamp the data are the spectrograms from both detectors. The labels will probably have to take into account both spectrograms, and it is not obvious what is the best way to do this, so the first step of this followup should probably generating the spectrograms from both detectors for each time-stamp in the Gravity Spy data set and determining their best label as a pair.
- Training:
