...
detector = 'L'
injected_path = Path('/storage/fast/users/tommaria/data/conditioned_data/16KHZ/' + detector + '1/injected/tomove')

start_time = time.time()

folders = [d for d in sorted(listdir(injected_path)) if (isdir(join(injected_path, d))) and d.startswith('injected')]

for folder in folders:
	data_path = join(injected_path, folder)
	...
	# if inj_type == 'cg_inc' or inj_type == 'cg_double' or inj_type == 'cg' or inj_type == 'wn':
