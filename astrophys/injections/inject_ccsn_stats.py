...
for file_idx in [11, 13, 14, 16, 40, 41, 42, 60, 61, 62]: # abdikamalov
# for file_idx in [0, 1]: # andersen
# for file_idx in [6, 7, 8]: # radice
		...
		for start in [0, 25, 50, 75]:
			...
			for D_kpc in [0.2]:
				...
				if ccsn_paper == 'abdikamalov':
					fname = 'injected-' + ccsn_paper + '-' + '.'.join(ccsn_file.split('.')[2:]) + '-D-' + str(D_kpc) + \
							'-' + str(times_s).zfill(4) + '-' + str(times_e-1).zfill(4) +'.hdf5'

				elif ccsn_paper == 'andersen':
					fname = 'injected-' + ccsn_paper + '-' + '-'.join(ccsn_file.split('.')[0].split('_')[1:]) + '-D-' + str(D_kpc) + \
							'-' + str(times_s).zfill(4) + '-' + str(times_e-1).zfill(4) + '.hdf5'

				elif ccsn_paper == 'radice':
					fname = 'injected-' + ccsn_paper + '-' + ccsn_file.split('.')[0] + '-D-' + str(D_kpc) + \
							'-' + str(times_s).zfill(4) + '-' + str(times_e-1).zfill(4) +'.hdf5'
