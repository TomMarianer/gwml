...
from inject_tools_backup import *
...
# 			inj_params = {'f0': f_inj, 'Q': Q, 'A': A, 'alpha': alpha}
# 
# if inj_type == 'rd':
...
# 		for alpha in parameters['alpha']:
# 			inj_params = {'f0': f_inj, 'tau': tau, 'A': A, 'alpha': alpha}
# 
# if inj_type == 'ga':
...
# 		for alpha in parameters['alpha']:
# 			inj_params = {'tau': tau, 'A': A, 'alpha': alpha}
# 
# if inj_type == 'cg':
# 	params = pd.read_csv(join(params_path, inj_type + '_params_csv.csv'), usecols=['f0', 'Q', 'A'])
# 	for f_inj, Q, A in zip(params['f0'][num_s:num_e], params['Q'][num_s:num_e], params['A'][num_s:num_e]):
...
# 		for alpha in parameters['alpha']:
# 			inj_params = {'f_low': f_low, 'f_high': f_high, 'tau': tau, 'A': A, 'alpha': alpha}
# 
			results = pool.starmap(load_inject_condition, 
								   [(t[0], t[1], t[2], inj_type, inj_params, local, Tc, To, fw, window, detector, qtrans, qsplit, dT) for t in times_par])
