...
def condition_chunks(t_i, t_f, local=False, Tc=16, To=2, fw=2048, window='tukey', detector='H', 
	...
	# print(chunks)
	cond_data_list = []
	# for chunk in tqdm_notebook(chunks, ncols=80):
	...
	for chunk in chunks:
		fname = 'conditioned-chunk-' + str(chunk[0]) + '-' + str(chunk[1]) + '.hdf5'
		...
		if local:
			...
			# load data from losc
			try:
				data = TimeSeries.fetch_open_data(detector + '1', *chunk, sample_rate=fw, verbose=False, cache=True)
			except:
				return
		...
		if save:
			...
			cond_data_list.extend(cond_data)
		else:
			cond_data_list.append(cond_data)
